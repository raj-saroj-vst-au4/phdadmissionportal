...
if ($status === 'No') {
    $und = (int)one("SELECT COUNT(*) c FROM candidates WHERE intake_id=? AND is_international=0 AND screening_status IN ('Pending','Doubtful')", [$intake['id']])['c'];
    if ($und > 0) {
    ...
    $missing = all("SELECT dept_reg_no FROM candidates WHERE intake_id=? AND is_international=0 AND screening_status='No' AND (remark IS NULL OR TRIM(remark)='') ORDER BY serial_no, id", [$intake['id']]);
    if ($missing) {
...
$rows = all("SELECT serial_no, dept_reg_no, name, birth_category, gender, categories_applied,
             ...
             FROM candidates WHERE intake_id=? AND is_international=0 AND screening_status=?
             ORDER BY serial_no, id", [$intake['id'], $status]);
foreach ($rows as &$r) $r['categories_applied'] = normalize_categories_applied($r['categories_applied']);
unset($r);
json_out($rows);
